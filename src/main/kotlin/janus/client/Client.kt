...
private fun sumFilesSize(plans: Collection<SyncPlan>): Long {
    return plans.sumOf { sumFilesSize(it) }
}
...
private suspend fun uploadFiles(conn: JanusProtocolConnection, workspace: Config.WorkspaceConfig, plan: SyncPlan) {
    ...
    while (pendingPlans.first.isNotEmpty()) {
        for (plan in pendingPlans.first) {
            pendingPlans.second += plan.children
        ...
        pendingPlans = Pair(pendingPlans.second, pendingPlans.first)
    }
...
private suspend fun getSvrToLocalTimeDiffMillis(conn: JanusProtocolConnection): Long {
    ...
    return svrToLocalTimeDiffMillis
}
...
private suspend fun getLocalAndRemoteFileTrees(conn: JanusProtocolConnection, workspace: Config.WorkspaceConfig): Pair<FileTree, FileTree> {
    ...
    return Pair(localFileTree!!, serverFileTree!!)
}


private fun buildSyncPlans(localTree: FileTree, remoteTree: FileTree, remoteToLocalTimeDiffMillis: Long): List<SyncPlan> {
    val syncPlans: List<SyncPlan>
    val buildSyncPlanCost = measureTime {
        syncPlans = SyncPlan.build(
            local = localTree,
            remote = remoteTree,
            remoteLocalTimeDiffMillis = remoteToLocalTimeDiffMillis
        ).toList()
    }
...
suspend fun runClient(config: Config): Int {
    ...
    JanusProtocolConnection(channel).use { conn ->
        ...
        val syncPlans: List<SyncPlan> = buildSyncPlans(
            localFileTree,
            serverFileTree,
            svrToLocalTimeDiffMillis
        )
